...
- Editor page boundary: `.editor-with-toc` has canvas background (`--tt-canvas-color`); `.simple-editor-content` has border, rounded corners (`--tt-radius-sm`), box-shadow (`--tt-page-shadow`), top/bottom margin, and `overflow: hidden`; fold crinkle extends edge-to-edge via negative margins (`-3rem` desktop, `-1.5rem` mobile) clipped by parent overflow; mobile override removes border/shadow/margin for edge-to-edge display
- TOC sidebar: heading-level typography hierarchy (H1: weight 600 full color, H2: weight 500, H3+: weight 400 muted); H1 sections separated by border-top + spacing; active heading gets brand-colored left border + tinted background; fold-aware (hides folded items); chevron hover with circular background
