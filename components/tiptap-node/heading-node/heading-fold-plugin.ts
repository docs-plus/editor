...
const stopFoldWidgetEvent = (e: Event) =>
...
export function findSectionDom(
  ...
  for (let i = 0; i < doc.content.childCount; i++) {
    ...
    if (
      ...
      child.attrs["data-toc-id"] === headingId
    ) {
...
function buildFoldDecorations(
  ...
  for (let i = 0; i < doc.content.childCount; i++) {
    ...
    const tocId = child.attrs["data-toc-id"] as string;
    ...
    decorations.push(
      Decoration.node(pos, pos + child.nodeSize, { class: headingClass }),
    );
    ...
    if (!isNested) {
      ...
      decorations.push(
        Decoration.widget(
          widgetPos,
          (view) => createFoldWidget(tocId, height, view),
          {
            key: `fold-${tocId}`,
            side: 1,
...
function pruneStaleIds(
  ...
  doc.forEach((node) => {
    if (node.type.name === "heading" && node.attrs["data-toc-id"]) {
      liveIds.add(node.attrs["data-toc-id"] as string);
    }
  });
...
function isPositionFolded(
  ...
  for (let i = 0; i < doc.content.childCount; i++) {
    ...
    if (
      child.type.name === "heading" &&
      foldedIds.has(child.attrs["data-toc-id"] as string)
    ) {
      const section = computeSection(
        doc,
        nodePos,
        child.attrs.level as number,
        i,
      );
...
export function createHeadingFoldPlugin(
  ...
  return new Plugin<HeadingFoldState>({
    ...
    props: {
      decorations(state) {
        return (
          headingFoldPluginKey.getState(state)?.decos ?? DecorationSet.empty
        );
      },
